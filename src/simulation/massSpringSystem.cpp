...
MassSpringSystem::MassSpringSystem()
    : isDrawingCube(true),
      ...
      position(Eigen::Vector4f(0.0f, 5.0f, 0.0f, 0.0f)),
      gravity(Eigen::Vector4f(0.0f, -9.8f, 0.0f, 0.0f)) {
    initializeCube();
    reset();
}
...
void MassSpringSystem::setSpringCoef(const float springCoef) {
    for (int cubeIdx = 0; cubeIdx < cubeCount; cubeIdx++) {
        springCoefStruct = springCoef;
        springCoefShear = springCoef;
        springCoefBending = springCoef;
        cubes[cubeIdx].setSpringCoef(springCoef);
    }
}
...
void MassSpringSystem::setDamperCoef(const float damperCoef) {
    for (int cubeIdx = 0; cubeIdx < cubeCount; cubeIdx++) {
        damperCoefStruct = damperCoef;
        damperCoefShear = damperCoef;
        damperCoefBending = damperCoef;
        cubes[cubeIdx].setDamperCoef(damperCoef);
    }
}

void MassSpringSystem::setTerrain(std::unique_ptr<Terrain>&& _terrain) { this->terrain = std::move(_terrain); }

void MassSpringSystem::setIntegrator(std::unique_ptr<Integrator>&& _integrator) {
    this->integrator = std::move(_integrator);
...
void MassSpringSystem::initializeCube() {
    for (int cubeIdx = 0; cubeIdx < cubeCount; cubeIdx++) {
        Cube NewCube(Eigen::Vector4f(0.0f, cubeLength, 0.0f - (2.0f * cubeLength * cubeIdx), 0.0f), cubeLength,
