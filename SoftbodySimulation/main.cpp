...
// Current terrain
graphics::Rigidbody* currentTerrainGraphics = nullptr;
...
std::unique_ptr<graphics::Plane> g_Plane = nullptr;
// Sphere and wiresphere share same vertices
// But they have different render behavior based on
// graphics::Sphere::RenderMode::WIREFRAME or graphics::Sphere::RenderMode::FILLED
std::unique_ptr<graphics::Sphere> g_FilledSphere = nullptr;
std::unique_ptr<graphics::Sphere> g_WiredSphere = nullptr;
// Soft body cube's graphics
std::unique_ptr<graphics::SoftCube> g_cube = nullptr;
...
int main() {
    ...
    graphics::Program simpleRenderProgram;
    ...
    g_cube = std::make_unique<graphics::SoftCube>(particleSystem.getCubePointer(0));
    // The skybox
    graphics::SkyBox skybox;
    // Load data from assets
    {
        using pf = util::PathFinder;
        graphics::Shader shadowVertexShader(pf::find("Shader/shadow.vert"), GL_VERTEX_SHADER);
        graphics::Shader shadowFragmentShader(pf::find("Shader/shadow.frag"), GL_FRAGMENT_SHADER);
        graphics::Shader renderVertexShader(pf::find("Shader/render.vert"), GL_VERTEX_SHADER);
        graphics::Shader renderFragmentShader(pf::find("Shader/render.frag"), GL_FRAGMENT_SHADER);
        graphics::Shader particleVertexShader(pf::find("Shader/particle.vert"), GL_VERTEX_SHADER);
        graphics::Shader particleFragmentShader(pf::find("Shader/particle.frag"), GL_FRAGMENT_SHADER);
        graphics::Shader skyboxVertexShader(pf::find("Shader/skybox.vert"), GL_VERTEX_SHADER);
        graphics::Shader skyboxFragmentShader(pf::find("Shader/skybox.frag"), GL_FRAGMENT_SHADER);

        auto dice1 = std::make_shared<graphics::Texture>(pf::find("Texture/dice0.png"));
        auto dice2 = std::make_shared<graphics::Texture>(pf::find("Texture/dice1.png"));
        auto dice3 = std::make_shared<graphics::Texture>(pf::find("Texture/dice2.png"));
        auto dice4 = std::make_shared<graphics::Texture>(pf::find("Texture/dice3.png"));
        auto dice5 = std::make_shared<graphics::Texture>(pf::find("Texture/dice4.png"));
        auto dice6 = std::make_shared<graphics::Texture>(pf::find("Texture/dice5.png"));
        ...
        // Setup shaders, these objects can be destroyed after attachLinkShader()
        ...
        simpleRenderProgram.attachLinkShader(particleVertexShader, particleFragmentShader);
        skyboxRenderProgram.attachLinkShader(skyboxVertexShader, skyboxFragmentShader);
        // Ownership is passed to graphics::SoftCube, so these shared pointers
    ...
    // Setup light, uniforms are persisted.
    {
        ...
        renderProgram.setUniform<3>("lightPos", lightPosition);
    }
    util::Clock clock;
    clock.reset();
    while (!glfwWindowShouldClose(window)) {
        ...
        simulationTime = clock.timeElapsed();
        ...
        simpleRenderProgram.setUniform("VP", currentCamera->getViewWithProjectionMatrix());
        ...
        renderProgram.use();
        renderProgram.setUniform<3>("viewPos", currentCamera->getPosition());
...
GLFWwindow* initialize() {
    ...
    // Load texture and create graphics of terrain
    auto wood = std::make_shared<graphics::Texture>(util::PathFinder::find("Texture/wood.png"));
    g_Plane = std::make_unique<graphics::Plane>();
    g_Plane->setTexture(wood);
    g_WiredSphere = std::make_unique<graphics::Sphere>(graphics::Sphere::RenderMode::WIREFRAME);
    g_WiredSphere->setTexture(wood);
    g_FilledSphere = std::make_unique<graphics::Sphere>(graphics::Sphere::RenderMode::FILLED);
    g_FilledSphere->setTexture(wood);
    // Physics engine
...
void debugCameraKeyboard(GLFWwindow* window, int key, int, int action, int) {
    ...
    if (ImGui::Begin("Simulation Control Panel", &isUsingSimulationPanel)) {
        ...
        if (particleSystem.isSimulating) {
            ...
            if (ImGui::InputFloat("Spring Coef", &particleSystem.springCoefStruct, 10.0f, 1.0f)) {
                ...
                particleSystem.setSpringCoef(particleSystem.springCoefStruct);
            }
            if (ImGui::InputFloat("Damper Coef", &particleSystem.damperCoefStruct, 10.0, 1.0)) {
                ...
                particleSystem.setDamperCoef(particleSystem.damperCoefStruct);
            }
